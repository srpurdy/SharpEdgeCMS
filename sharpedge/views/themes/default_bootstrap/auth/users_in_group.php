			...
			<table class="table table-striped text-size">
				...
				<?php foreach ($users_in_group->result() as $id):?>
					<tr>
						...
						<td>
						<a class="btn btn-danger" href="<?php echo site_url()?>/user_admin/delete_user_in_group/<?php echo $this->uri->segment(3);?>/<?php echo $id->id?>" onClick="return confirm('Are you sure you want to Delete this item?')"><i class="icon-trash icon-white"></i> <?php echo $this->lang->line('label_delete');?></a>
						</td>
					</tr>
				<?php endforeach;?>
			</table>
