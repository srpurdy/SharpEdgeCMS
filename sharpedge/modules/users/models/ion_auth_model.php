...
class Ion_auth_model extends CI_Model
{
	...
	public function get_users_groups_new()
		{
		$groups = $this->db
			->select('
				groups.name,
	...
	public function update_group_permissions()
		...
		if($module_id == '')
			...
			{
			$this->db->where('module_permissions.group_id', $group_id[0]);
	...
	public function get_modules()
		...
		$modules = $this->db
			->where('modules.is_admin', 'Y')
			//->or_where('modules.id = module_permissions.module_id')
			//->or_where('module_permissions.group_id', $this->uri->segment(3))
			->select("
				modules.id,
				modules.name,
				(select module_permissions.read from module_permissions where module_permissions.module_id = modules.id AND module_permissions.group_id = '$seg') as read_access,
				(select module_permissions.write from module_permissions where module_permissions.module_id = modules.id AND module_permissions.group_id = '$seg') as write_access,
				(select module_permissions.delete from module_permissions where module_permissions.module_id = modules.id AND module_permissions.group_id = '$seg') as delete_access
			")
			->from('modules')
			->get();
		return $modules;
	...
	public function users_in_group($id)
	{
	$users_group = $this->db
	...
	public function delete_user_in_group($group_id,$user_id)
	{
	...
	public function login_remembered_user()
	{
	    ...
	    //get the user
	    $this->trigger_events('extra_where');
	    $query = $this->db->select('users.username, users.email, users.id, users.password, users.last_login, users.first_name, users.last_name, groups.name, profile_fields.display_name, profile_fields.nickname')
	...
	{
	$this->db->where('id', $id);
	$get_edit = $this->db->get('users');
	return $get_edit;
	}
	
	public function edit_get_user()
	{
