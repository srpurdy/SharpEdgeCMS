...
class sharpedge_install {
	...
	public function install($data)
	{
		...
		$data['user_password'] = $salt . substr(sha1($salt . $data['user_password']), 0, -10);

		// Get the SQL for the default data and parse it
		$user_sql = file_get_contents('./sql/sharpedge_default.sql');
		...
		$user_sql = str_replace('{SALT}', $salt, $user_sql);
	...
	function write_db_file($database)
	{
		...
		$handle 	= @fopen('../sharpedge/config/database.php','w+');
